...
//		xSemaphoreTake(m_semaphore, portMAX_DELAY);
		xSemaphoreTake(m_semaphore, 15000UL);  // TEB, Oct-24-2019. Prevent endless cpu hangs.
	}
...
bool FreeRTOS::Semaphore::take(std::string owner) {
	...
	if (m_usePthreads) {
		...
		rc = ::xSemaphoreTake(m_semaphore, 15000UL) == pdTRUE;    // TEB, Oct-29-2019. Prevent endless cpu hangs.
	}
	m_owner = owner;
	if (rc) {
